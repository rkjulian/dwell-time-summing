# Flow Injection DT Rate Average Correlations.R
...
# user supplied input
Conc <- '1000ng' # choices: '1500ng', '1000ng', '500ng', '250ng', '50ng'
Chan <- '2x10'   # choices: '2', '2x4', '2x10', number of cmpds x number dwells
Dwell <- '6ms' # choices: '96ms' '21ms'  '6ms'
...
for(rep in 1:3){
  ...
  mu[rep,] <- colMeans(rates)
  var[rep,] <- apply(rates,2,var)
  covar <- covar + cov(rates)/3
  corr <- corr + cor(rates)/3
}
  ...
  
{
  ...
  if(pow < 2) pow <- 0
  cat('\nRate Parameters with Replicates Averaged:',
      ' \n\tConc = ',Conc,
      '; Chan =  ',Chan,
      '; Dwell = ',Dwell,
      '; Pause = ',Pause,
  ...
  cat('Mean Diagonal =',mean(cordiag),'\n')
